...
import ExploreCardroz from "components/Marketplace/ExploreCardroz";
...
export default function Profilex() {
  ...
  const [badgeyok, setBadgeyok] = useState(false);
...
>>>>>>> refs/remotes/origin/main
    ...
    const ourBadges = await contract.getOwnedBadges();
    if (ourBadges.length < 1) {
      setBadgeyok(true);
    } else {
      setBadgeyok(false);
    }
    /**
     * market keşfet kısmı da aynı
     * struct Collection {
        address creator;
        uint collectionID;
        uint count;
  ...
  return (
    <>
      ...
      <div className="bg-[#111827] overflow-y-auto h-screen flex justify-between">
        <Header />
        <div>
          <div className=" mt-[120px] flex justify-center bg-slate-700 ml-[90px] rounded-lg p-2 w-auto ">
            <div className=" flex justify-center grid-cols-2 ">
              <div className=" bg-slate-600 rounded-lg w-[300px] hover:scale-105 ease-in duration-100">
                <Link href="/profile">
                  <a>
              ...
              <div className="bg-slate-600 rounded-lg w-[300px] ml-[70px] border-2 border-cyan-600 hover:scale-105 ease-in duration-100">
                <Link href="/rozetlerim">
                  <a>
                    <p className="font-poppins flex justify-center font-bold dark:text-white">
                      Rozetlerim
                    </p>
          ...
          <div className="items-center grid grid-cols-3 grid-rows-2 min-h-screen h-auto w-auto ml-[90px]">
            {badgeyok ? (
              ...
              badges?.map((rozetler) => <ExploreCardroz rozet={rozetler} />)
            )}
          </div>
        </div>
            ...
            <div className=" h-150  bg-slate-700 rounded-xl flex justify-center p-4 mt-2">
              ...
              <div className="o dive overflow-auto overflow-x-hidden">
                ...
                <div className="grid grid-cols-3 grid-rows-2 mt-2 place-items-stretch">
                  <Link href="/succubus">
                    <a>
                      <p className=" mr-2 mb-2 w-auto text-white font-roboto h-auto m-auto bg-slate-600 rounded-xl flex justify-center p-1 hover:scale-105 ease-in duration-100">
                        Blockchain
                      </p>
                    </a>
                  </Link>
                  <Link href="/succubus">
                    <a>
                      <p className=" mr-2 w-auto h-auto text-white font-roboto m-auto bg-slate-600 rounded-xl flex justify-center p-1 hover:scale-105 ease-in duration-100">
                        Blockchain
                      </p>
                    </a>
                  </Link>
